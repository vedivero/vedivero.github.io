...
export const resume = {
  ...
  introduce: [
    ...
    "전자정부 표준프레임워크로 커리어를 시작해 지금은 벡터 검색과 LLM 서빙까지 다루고 있습니다. 익숙하지 않은 기술이라도 문제를 푸는 데 더 맞는 선택이라면 파고들어 제 것으로 만들어 왔습니다.",
    "기술보다 문제를 먼저 봅니다. 문자열 매칭으로는 부족한 검색에 벡터 검색을, 응답이 느린 지도 서비스에 WMTS 전환을 적용한 것처럼, 왜 이 방법이어야 하는지 설명할 수 있는 선택을 하려 합니다.",
    "개발만이 아니라 클라이언트와 직접 미팅하며 요구사항을 파악하고 개발 방향을 함께 정해 왔습니다. 기술적 제약과 업무상의 필요를 조율하는 과정에서, 서로의 맥락을 맞추는 일이 결국 결과물의 완성도를 좌우한다고 배웠습니다.",
